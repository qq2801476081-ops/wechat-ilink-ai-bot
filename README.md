...
- 每分钟由 Cron Trigger 启动一次最长 55 秒的循环长轮询，活跃窗口内可实现秒级响应。
...
1. **Bot 账号端**：登录微信小号，设置一个真人风格的头像和昵称（如“小助理”）。
2. **你的微信端**：长按 Bot 聊天窗口 → 设置备注和标签 → 修改备注名和备注头像。
   - 这样只有你自己看到自定义形象，不影响其他添加 Bot 的人。

> iLink 协议本身不提供修改 Bot 名称或头像的接口；是否能修改备注头像，以当前微信客户端显示的功能为准。

### 调整 AI 性格（可选）

修改 `src/ai.ts` 中的 `SYSTEM_PROMPT` 变量，可以改变 Bot 的性格：

- 温柔型：使用安慰、关心的语气（当前默认）。
- 搞笑型：使用段子手、吐槽的语气。
- 高冷型：使用简短、酷酷的语气。

当前 AI 使用“有点毒舌但内心温柔”的真人好友设定，`temperature` 为 `0.85`，`max_tokens` 为 `80`，让回复更自然、简短且有变化。
...
Bot 通过 Cron Trigger 每分钟启动一次循环长轮询，单次执行最多 55 秒，每轮网络等待约 35 秒且几乎不消耗 CPU。如果超过 24 小时没有用户发消息，iLink 的 `context_token` 会过期，Bot 将无法回复。此时需要重新访问 `/setup` 扫码登录。建议每天与 Bot 互动一次，或接受偶尔需要重新登录的事实。
